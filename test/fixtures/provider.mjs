...
const unique = process.env.RELAY_FIXTURE_UNIQUE_SESSIONS === '1';
const resumeIndex = process.argv.findIndex(arg => arg === 'resume' || arg === '--resume');
const session = unique ? (resumeIndex >= 0 ? process.argv[resumeIndex + 1] : randomUUID()) : `${agent}-fixture-session`;
let turns = 1;
if (unique) {
  const counter = '.fixture-' + session + '.json';
  try { turns += JSON.parse(readFileSync(counter, 'utf8')).turns; } catch {}
...
if (agent === 'codex') emit({ type: 'turn.completed', usage: { input_tokens: 100 * turns, cached_input_tokens: 80 * turns, output_tokens: 20 * turns } });
